from referee.game import BOARD_N, Board, PLACEMENT_TURNS, INITIAL_STACK_HEIGHT 
...
MAX_TOKENS = PLACEMENT_TURNS // 2 * INITIAL_STACK_HEIGHT
...
def minimax_evluation_function(
    ...
    for my_coord, my_height in my_positions.items():
        my_score += 1.5* evaluate_eat(opp_positions, my_coord, my_height)
        my_score += 2.0*evaluate_cascade_board(opp_positions, my_positions, my_coord, my_height, board)
    
    my_score += my_total_height / MAX_TOKENS

    for enemy_coord, enemy_height in opp_positions.items():
        opponent_score += 1.5* evaluate_eat(my_positions, enemy_coord, enemy_height)
        opponent_score += 2.0*evaluate_cascade_board(my_positions, opp_positions, enemy_coord, enemy_height, board)
    
    opponent_score += opp_total_height / MAX_TOKENS
...
def evaluate_eat(
    ...
    for (opponent_r, opponent_c), opponent_height in opponents_stacks.items():
        distance = manhanttan_distance(Coord(curr_coord_r, curr_coord_c), Coord(opponent_r, opponent_c))
...
def evaluate_cascade_board(
    ...
    Normalise the range to be in within [-1, 1] 
    """
    ...
    for direction in CARDINAL_DIRECTIONS:
        ...
        for i in range(1, curr_cell_height + 1):
            ...
            if not board._is_within_bounds(coord_r, coord_c):
                token_lost =  curr_cell_height - i + 1
                out_of_bound_penalty -= ((token_lost) / MAX_TOKENS)
                break
        ...
        # how much enemy has lost compared to our lost     
        push_score = (enemy_total_lost - my_total_lost) / MAX_TOKENS
                ...
                for adj_dir in CARDINAL_DIRECTIONS:
                    ...
                    if (adj_r, adj_c) in updated_opponents:
                        ...
                        if my_height >= enemy_height:
                            adj_score += (1 + height_diff) / MAX_TOKENS
                        else:
                            adj_score -= (1 + height_diff) / MAX_TOKENS
                        ...
                        for j in range(1, my_height + 1):
                            ...
                            if (target_r, target_c) in updated_opponents:
                                ...
                                if not board._is_within_bounds(next_r, next_c):
                                    cascade_opp_score += (updated_opponents[(target_r, target_c)] * (1/ j) / MAX_TOKENS)
                                    break

                                break

        curr_score = 1 * push_score + 0.8* adj_score + 0.5* cascade_opp_score + 1.0 * out_of_bound_penalty
